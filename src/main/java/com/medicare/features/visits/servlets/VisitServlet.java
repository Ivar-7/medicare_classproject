...
public class VisitServlet extends HttpServlet {
  ...
  private void saveVisit(HttpServletRequest request, HttpServletResponse response, User currentUser)
    ...
    if (regNumberRaw == null || regNumberRaw.isBlank()) {
      forwardWithError(request, response, "Student is required.", visitIdRaw, 0, doctorIdRaw,
          symptoms);
      return;
    }

    try {
      regNumber = Integer.parseInt(regNumberRaw);
    } catch (NumberFormatException e) {
      forwardWithError(request, response, "Invalid student registration number.", visitIdRaw, 0,
    ...
    visit.setStatus(completed ? MedicalVisit.Status.Completed : MedicalVisit.Status.Ongoing);
